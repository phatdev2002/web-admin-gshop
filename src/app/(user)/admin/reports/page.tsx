...
const ReportPage = () => {
  ...
  // Chart data format cho biểu đồ line chart
  const monthlyRevenueChartData = Object.entries(monthlyRevenue ?? {}).map(
    ([month, value]) => ({
      name: month,
      total: value || 0,
    })
    ...
    const fetchMonthlyRevenueData = async () => {
      try {
        ...
        if (result.status) {
          ...
          setMonthlyRevenue(
            Object.fromEntries(
              sortedData.map(({ name, totalRevenue }) => [name, totalRevenue])
            )
          );
    ...
    <div>
      ...
      <section className="gap-4 transition-all lg:grid-cols-[7fr_3fr] grid-cols-1 lg:grid">
        <section className="grid grid-cols-1 gap-4 transition-all">
          ...
          <CardContent>
            <p className="font-semibold">Thống kê doanh thu theo tháng</p>
            <p className="pb-4 text-xs text-gray-500">Biểu đồ đường doanh thu trong năm</p>
          ...
          <CardContent>
            <p className="font-semibold">Thống kê doanh thu theo ngày</p>
            <p className="pb-4 text-xs text-gray-500">
              Biểu đồ đường doanh thu trong 7 ngày gần nhất
            </p>
            <LineChart
              data={Object.entries(
                revenueData.dailyRevenueLast7Days || {}
              )
                .reverse()
                .map(([date, value]) => ({
                  name: date,
          ...
          <CardContent>
            <section>
              <p className="font-semibold pb-2">
                Tỉ lệ loại Gundam trên tổng số đã bán
              </p>
              <p className="pb-4 text-xs text-gray-500">
